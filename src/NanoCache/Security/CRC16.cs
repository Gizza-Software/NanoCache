...
namespace NanoCache.Security
{
    public static class CRC16
    {
        ...
        private static ushort ComputeChecksum(byte skipBytesFirst, byte SkipBytesLast, params byte[] buffer)
        {
            ...
            {
                crc = (ushort)(crc >> 8 ^ _crc16Table[(crc ^ buffer[i]) & 0xff]);
            }
            return crc;
        }

        public static void ComputeCRC16(byte[] buffer, ref byte crc1, ref byte crc2)
        {
        ...
        public static bool CheckCRC16(byte[] buffer, byte crc1, byte crc2)
        {
            byte skipBytesFirst = 0, SkipBytesLast = 0;
            byte[] result = BitConverter.GetBytes(ComputeChecksum(skipBytesFirst, SkipBytesLast, buffer));
            return result[0] == crc1 && result[1] == crc2;
        }
    }
}
